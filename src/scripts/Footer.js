    ...
    <footer className="font-display text-[#E7E5DF]">
      ...
      <div className="relative" style={FOOTER_PATTERN}>
        ...
        <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 py-14 sm:grid-cols-2 lg:grid-cols-4">
          {/* Brand */}
          <div>
            <a href="/" aria-label="KC Trailer Rentals — home" className={"inline-flex items-center rounded " + FOCUS}>
              <img src={logoUrl} alt="KC Trailer Rentals" className="h-24 w-auto" />
            </a>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {TRAILERS.map((t) => (
                <li key={t.href}><FooterLink href={t.href}>{t.name}</FooterLink></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {COMPANY.map((c) => (
                <li key={c.href}><FooterLink href={c.href}>{c.name}</FooterLink></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3.5 text-[14px]">
              <li>
                <a href={PHONE_HREF} className={"inline-flex items-center gap-2.5 rounded font-semibold text-white transition-colors duration-[var(--dur-micro)] ease-[var(--ease-out)] hover:text-[#D7282F] " + FOCUS}>
                  <PhoneIcon className="h-4 w-4 text-[#D7282F]" />{PHONE_DISPLAY}
                </a>
              </li>
              <li>
                <a href={EMAIL_HREF} className={"inline-flex items-center gap-2.5 break-all rounded text-[#C7CDD3] transition-colors duration-[var(--dur-micro)] ease-[var(--ease-out)] hover:text-[#D7282F] " + FOCUS}>
                  <MailIcon className="h-4 w-4 shrink-0 text-[#D7282F]" />{EMAIL_DISPLAY}
                </a>
              </li>
              <li className="inline-flex items-start gap-2.5 text-[#C7CDD3]">
                <MapPinIcon className="mt-0.5 h-4 w-4 shrink-0 text-[#D7282F]" />{LOCATION}
              </li>
            </ul>
      ...
      <div className="bg-[#11161B] text-[#9AA4AE]">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-6 py-4 text-[12px] sm:flex-row">
          ...
          <span className="flex items-center gap-4">
            <a href="/rental-info" className={"rounded transition-colors duration-[var(--dur-micro)] ease-[var(--ease-out)] hover:text-[#D7282F] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EE3A41] focus-visible:ring-offset-2 focus-visible:ring-offset-[#11161B]"}>Rental Info</a>
            <a href="/contact" className={"rounded transition-colors duration-[var(--dur-micro)] ease-[var(--ease-out)] hover:text-[#D7282F] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#EE3A41] focus-visible:ring-offset-2 focus-visible:ring-offset-[#11161B]"}>Contact</a>
          </span>
        </div>
      </div>
    </footer>
...
function FooterLink({ href, children }) {
  return (
    <a href={href} className={"rounded text-[14px] text-[#C7CDD3] transition-colors duration-[var(--dur-micro)] ease-[var(--ease-out)] hover:text-[#D7282F] " + FOCUS}>
      {children}
    </a>
  )
}
...
function MapPinIcon({ className }) {
  return (<svg className={className} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 21s7-5.5 7-11a7 7 0 10-14 0c0 5.5 7 11 7 11z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" /><circle cx="12" cy="10" r="2.5" stroke="currentColor" strokeWidth="1.8" /></svg>)
}
...
function FacebookIcon({ className }) {
  return (<svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13.5 21v-7h2.3l.4-2.7h-2.7V9.6c0-.8.2-1.3 1.3-1.3h1.5V5.9c-.3 0-1.2-.1-2.2-.1-2.2 0-3.6 1.3-3.6 3.7v2.1H8.2V14h2.5v7h2.8z" /></svg>)
}